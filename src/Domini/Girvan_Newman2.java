 ...
 */
public class Girvan_Newman2 extends Algoritmo2{
    ...
    private GrafoNodoArista data_graph;
    //Referencia al grafo que se usa internamente en este algoritmo
    private GrafoNodoArista alg_graph;
    ...
    public Girvan_Newman2(Entrada2 i, Salida2 o) throws Exception
    {
        super(i, o);
        ...
        alg_graph = new GrafoNodoArista(data_graph);
    ...
    private void ejecutar_iteración(GrafoNodoArista g) throws Exception {
        ...
        for (int i = 0; i < N; ++i)
        {
            ...
            while(!Q.isEmpty())
            {
                ...
                if (!ref_v.visited) {
                    ...
                    route.add(v);
                    int leaf_index = 0;
                    Domini.Node vv = g.obtVertice(v);
                    List<Integer> al = g.nodosSalida(g.obtVertice(v));
                    //System.out.print("      adyacentes:");
                    //Para todos los nodos adyacentes a v
                    for (int aux : al) {
                        Node ref_aux = node.get(aux);
                        Domini.Node vaux = g.obtVertice(aux);
                        double dist = data_graph.pesoAristasVertice(vv, vaux); //dist es el peso de la arista v -> aux
                        //Dado que alg_graph es una copia de data_graph, necesitamos que, al principio, todas las aristas de alg_graph tengan peso = 0
                        if (i == 0 && dist == alg_graph.pesoAristasVertice(vv, vaux )) alg_graph.modificarArista(new Edge(vv, vaux, dist), 0);
            ...
            for (int z = 0; z < endRoute; ++z)
            {
                ...
                if (golf.parent.size() > 0) {
                    ...
                    for (int inode : golf.parent) {
                        ...
                        Domini.Node vup = g.obtVertice(inode);
                        double multiplier = up.weight/golf.weight;
                        double myWeight = (1 + golf.down_total) * multiplier;
                        double dependency = alg_graph.pesoAristasVertice(vup, vp);
                        double rel = dependency + myWeight / data_graph.pesoAristasVertice(vup, vp);
                        alg_graph.modificarArista(new Edge (vup, vp, dependency), rel);
        ...
        for (int x = 0; x < eixos.size(); x++) {
            ...
            Domini.Node va1 = g.obtVertice(a1);
            Domini.Node va2 = g.obtVertice(a2);

            if(alg_graph.existeAlgunaArista(va1, va2) && alg_graph.existeAlgunaArista(va2, va1)) {
                alg_graph.eliminarAristas(va1, va2);
                alg_graph.eliminarAristas(va2, va1);
        ...
        for (int iii = 0; iii < N; ++iii)
        {
            ...
            if (uno.component == -1)
            {
                ++cc2;
                uno.component = cc2;
                comm = new HashSet<Congresista>();
                comm.add((Congresista) g.obtVertice(iii));
            ...
            while(!Q.isEmpty())
            {
                ...
                Node ref_v = node.get(v);
                Domini.Node vref_v = g.obtVertice(v);
                if (!ref_v.visited) {
                    ref_v.visited = true;
                    if (ref_v.component == -1)
                    {
                        ref_v.component = cc2;
                        comm.add((Congresista) vref_v);
                    }

                    route.add(v);
                    int leaf_index = 0;
                    List<Integer> al = alg_graph.nodosSalida(vref_v);
                    for (int aux : al) { //Para todos los nodos adyacentes a V[v]

                        Node ref_aux = node.get(aux);
                        Domini.Node vref_aux = g.obtVertice(aux);
                        double dist = data_graph.pesoAristasVertice(vref_v, vref_aux); //dist es el peso de v -> aux
                        if (!ref_aux.visited) {
