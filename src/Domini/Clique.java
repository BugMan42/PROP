...
public class Clique extends Algoritmo {
    ...
    private void fase1(comunidades c) throws Exception{
        for (Iterator it = g.consultarVertices().iterator(); it.hasNext(); ) {
            ...
            out.agregarMensaje("Tratando nodo num: " + Integer.toString(i));
            int m = g.degreeSalida(i);
            out.agregarMensaje("Grado de: " + Integer.toString(i) + ": " + Integer.toString(m));
            if (m + 1 >= k) {
                List<Integer> candidatos = g.nodosSalida(i).subList(index_sublista(i), m);
                out.agregarMensaje("Candidatos");
                for (int j = 0; j < candidatos.size(); ++j) out.agregarMensaje("Soy el candidato numero " + Integer.toString(candidatos.get(j)));
                for (Iterator it2 = candidatos.iterator(); it2.hasNext(); ) {
                    ...
                    out.agregarMensaje("Lista recursividad");
                    for (int j = 0; j < l.size(); ++j) out.agregarMensaje("Soy el candidato numero " + Integer.toString(candidatos.get(j)));
                    cliqueOneNode(kc, k - 2, l);
                    if (kc.size() > 0) {
                        out.agregarMensaje("En el nodo num: " + Integer.toString(i) + " se ha creado una clique");
                        c.agregar_clique(kc);
                    }
                    out.agregarMensaje("Acabo de quitar el vertice " + Integer.toString(v));
                }
            }
        }
    }
    ...
    private void buscarCom(comunidades c, k_clique kc, Set<Integer> s, int i) {
        out.agregarMensaje("El clique num: " + Integer.toString(i) + " no tiene comunidad aun");
        kc.mod_num();
        boolean agregado = false;
        for (int j = 0; j < c.size(); ++j) {
            if (i != j) {
                k_clique kc1 = c.obt_clique(j);
                out.agregarMensaje("Mirando clique num: " + Integer.toString(j) + " para relacionarlo con " + Integer.toString(i));
                if (kc1.obtNum() == 0) {
                    out.agregarMensaje("El clique num: " + Integer.toString(j) + " no tiene comunidad aun");
                    ArrayList<Integer> aux = new ArrayList<Integer>(kc.lista());
                    aux.retainAll(kc1.lista());
                    out.agregarMensaje("El clique num: " + Integer.toString(j) + " tiene con clique num: " + Integer.toString(i) + " exactamente estos nodos en comun " + Integer.toString(aux.size()));
                    if (aux.size() == k - 1) {
                        out.agregarMensaje("El clique num: " + Integer.toString(j) + " esta en la misma comunidad que " + Integer.toString(i));
