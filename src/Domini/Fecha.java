...
public class Fecha {
    ...
    private static boolean Correcto(int dia, int mes, int any) {
        ...
        if (mes == 2) {
            ...
            if (dia == 29 && ((any % 4 != 0 || any % 100 == 0) && any % 400 != 0))
                throw new IllegalArgumentException("Dia 29 de febrero solo en años bisiestos");
        }
    ...
    public Fecha(String data) throws PatternSyntaxException {
        if (data.equals("")) throw new IllegalArgumentException("La fecha no puede ser vacía");
        String[] aux = data.split("/");
    ...
    public boolean igual(String date) {
        if (date.equals("")) throw new IllegalArgumentException("La fecha no puede ser vacía");
